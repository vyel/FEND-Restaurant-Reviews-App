...
window.initMap = () => {
  fetchRestaurantFromURL((error, restaurant) => {
    if (error) { // Got an error!
      ...
      //source: https://stackoverflow.com/questions/49012240/google-maps-js-iframe-title
...
fillRestaurantHTML = (restaurant = self.restaurant) => {
  ...
  const website = document.getElementById('restaurant-website');
  link = document.createElement('a');
  link.href = `${restaurant.website}`;
  link.rel = "noopener noreferrer"
  link.target = "_blank"
  link.setAttribute("aria-label", `Go to ${restaurant.name} website`);
...
createReviewHTML = (review) => {
  ...
  // Create review header (reviwer name and date)
  const reviewHeader = document.createElement('h3');
  reviewHeader.classList.add('review-header');
  reviewHeader.tabIndex = 0;
